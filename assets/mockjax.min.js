...
function isMockDataEqual(mock,live){var identical=!0;if(typeof live==='string'){return $.isFunction(mock.test)?mock.test(live):mock===live}
...
if($.isFunction(handler.url.test)){if(!handler.url.test(requestSettings.url)){return null}}else{var namespace=handler.namespace||$.mockjaxSettings.namespace;if(!!namespace){var namespacedUrl=[namespace,handler.url].join('/');namespacedUrl=namespacedUrl.replace(/(\/+)/g,'/');handler.url=namespacedUrl}
var star=handler.url.indexOf('*');if(handler.url!==requestSettings.url&&star===-1||!new RegExp(handler.url.replace(/[-[\]{}()+?.,\\^$|#\s]/g,'\\$&').replace(/\*/g,'.+')).test(requestSettings.url)){return null}}
...
this.responseTimer=setTimeout(process,parseResponseTimeOpt(mockHandler.responseTime))}})}else{if(requestSettings.async===!1){process()}else{this.responseTimer=setTimeout(process,parseResponseTimeOpt(mockHandler.responseTime))}}}
function xhr(mockHandler,requestSettings,origSettings,origHandler){mockHandler=$.extend(!0,{},$.mockjaxSettings,mockHandler);if(typeof mockHandler.headers==='undefined'){mockHandler.headers={}}
...
requestSettings.url=requestSettings.url.replace(CALLBACK_REGEX,'='+jsonp+'$1');window[jsonp]=window[jsonp]||function(){jsonpSuccess(requestSettings,callbackContext,mockHandler);jsonpComplete(requestSettings,callbackContext);window[jsonp]=undefined;try{delete window[jsonp]}catch(e){}}}
...
unmockedAjaxCalls.push(origSettings);if($.mockjaxSettings.throwUnmocked===!0){throw new Error('AJAX not mocked: '+origSettings.url)}
...
$.extend({ajax:handleAjax});var DEFAULT_RESPONSE_TIME=500;$.mockjaxSettings={log:function(mockHandler,requestSettings){if(mockHandler.logging===!1||(typeof mockHandler.logging==='undefined'&&$.mockjaxSettings.logging===!1)){return}
if(window.console&&console.log){var message='MOCK '+requestSettings.type.toUpperCase()+': '+requestSettings.url;var request=$.ajaxSetup({},requestSettings);if(typeof console.log==='function'){console.log(message,request)}else{try{console.log(message+' '+JSON.stringify(request))}catch(e){console.log(message)}}}},logging:!0,namespace:null,status:200,statusText:'OK',responseTime:DEFAULT_RESPONSE_TIME,isTimeout:!1,throwUnmocked:!1,contentType:'text/plain',response:'',responseText:'',responseXML:'',proxy:'',proxyType:'GET',lastModified:null,etag:'',headers:{etag:'IJF@H#@923uf8023hFO@I#H#','content-type':'text/plain'}};$.mockjax=function(settings){var i=mockHandlers.length;mockHandlers[i]=settings;return i};$.mockjax.clear=function(i){if(i||i===0){mockHandlers[i]=null}else{mockHandlers=[]}
mockedAjaxCalls=[];unmockedAjaxCalls=[]};$.mockjax.handler=function(i){if(arguments.length===1){return mockHandlers[i]}};$.mockjax.mockedAjaxCalls=function(){return mockedAjaxCalls};$.mockjax.unfiredHandlers=function(){var results=[];for(var i=0,len=mockHandlers.length;i<len;i++){var handler=mockHandlers[i];if(handler!==null&&!handler.fired){results.push(handler)}}
